function [tc, results] = wrworkbenchdiabarm
%WRWORKBENCHDIABARM Creates a Wind River Workbench DIAB toolchain.

% Copyright 2013-2019 The MathWorks, Inc.

toolchain.Platforms  = {'win64', 'win32','glnxa64','maci64'};
toolchain.Versions   = {'5.9.4.0'};
...
toolName = 'Wind River Workbench DIAB';
...
% VxWorks DIAB Specific macros
tc.addMacro('VX_CPU_FAMILY','arm');
tc.addMacro('CPU','ARMARCH7');
tc.addMacro('TOOL_FAMILY','diab');
tc.addMacro('TOOL','diab');
tc.addMacro('CC_ARCH_SPEC','-tARMV7MV:rtp7');
...
archiver.setName(           [toolName, ' Librarian']);
archiver.setCommand(        'dar');
...
commonOpts = {...
    '$(CC_ARCH_SPEC)', '-Y I,+$(VSB_DIR)/usr/h/c++:$(VSB_DIR)/usr/h:$(WIND_DIAB_PATH)/include/diab',...
    '-Xansi', '-WDVSB_DIR=$(VSB_DIR)', '-W:c++:.CPP', '-Xfp-fast', '-Xforce-declarations', ...
    '-Xmake-dependency=0xd', '-Xlocals-on-stack', ...
    '-D_VX_CPU=_VX_$(CPU)','-D_VX_TOOL_FAMILY=$(TOOL_FAMILY)','-D_VX_TOOL=$(TOOL)',...
...
compilerOpts = [commonOpts, { tc.getBuildTool('C Compiler').getDirective('CompileFlag') }];

linkerOpts = [commonOpts, { ...
    '-tARMV7MV:rtp7',...
    '-L$(VSB_DIR)/usr/lib/common',...
    '-lstlstd',...
    '-Wl,-Bt=0x80000000', ...
    '-Wl,-rpath $(VSB_DIR)/usr/root/diab/bin'
    }];
